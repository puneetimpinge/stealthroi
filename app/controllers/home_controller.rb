class HomeController < ApplicationController
  ...
  def get_top_graph
    ...
    if params[:graphstoreindex] == "1"
      (start_time.to_i..end_time.to_i).step(step).each_with_index do |a,i|
          spend = current_user.fb_ads.where('created_at >= ? AND created_at < ? AND urldomain LIKE ?', Time.zone.at(a),Time.zone.at(a)+time_inc, "%viralstyle%").map(&:tot_spend).compact.sum.to_s
      ...
      (start_time.to_i..end_time.to_i).step(step).each_with_index do |a,i|
        spend = current_user.fb_ads.where('created_at >= ? AND created_at < ? AND urldomain like ?', Time.zone.at(a),Time.zone.at(a)+time_inc, "%teehood%").map(&:tot_spend).compact.sum.to_s
  ...
  def get_top_summary
    ...
    if params["parameters"]["graphstoreindex"] == "1"
      ...
      totalSpent = @spend.map(&:tot_spend).compact.sum.round(2)
